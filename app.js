...
let formEl = document.getElementById("formId"); /* Syntax error: Mismatch form ID */
let allDrinks = [];
...
Drink.prototype.renderTable = function () {
    ...
    tr.appendChild(priceTd); // Incorrect variable name
}
...
let hotchocolate = new Drink("hot chocolate", /* Typo error */ ["milk", "coffee", "ice", "sugar"], "./assets/mocha.png", true, false, 2)

function renderAll() {
    for (let i = 0; i <= allDrinks.length - 1; /* Logical error: Subtract drinks length by 1 to prevent overflow */ i++) {
        allDrinks[i].render();
...
function getData() {
    ...
    if (arrayData != null) {
        for (let i = allDrinks.lingth; i < arrayData.length; i++) {
